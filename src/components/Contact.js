      ...
      <div className='container grid justify-center grid-cols-1 sm:grid-cols-2 gap-5 lg:gap-7'>
        {ContactInfo.map((item, index) => (
          <a
            ...
            key={index}
            href={item.link}
            className='tracking-widest text-sm font-semibold hover:bg-emerald-600 hover:bg-opacity-30 border-2 border-emerald-600 rounded'
          >
            <div className='md:max-w-none overflow-hidden '>
              <div className='p-4 ease-in-out duration-300'>
                ...
                <h2 className='tracking-widest text-sm font-semibold text-emerald-600 hover:'>
                  {item.line}
                </h2>
              </div>
            </div>
